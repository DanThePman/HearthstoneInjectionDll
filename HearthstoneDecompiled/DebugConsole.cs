...
public class DebugConsole
{
    ...
    public void Init()
    {
        if (!this.m_initialized)
        {
            ...
            network.RegisterNetHandler(DebugConsoleCommand.PacketID.ID, new Network.NetHandler(this.OnCommandReceived), null);
            network.RegisterNetHandler(BobNetProto.DebugConsoleResponse.PacketID.ID, new Network.NetHandler(this.OnCommandResponseReceived), null);
    ...
    private void SendConsoleCmdToServer(string commandName, List<string> commandParams)
    {
        if (s_serverConsoleCallbackMap.ContainsKey(commandName))
        {
            ...
            if (!Network.SendConsoleCmdToServer(command))
            {
