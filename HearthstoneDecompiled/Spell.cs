...
public class Spell : MonoBehaviour
{
    ...
    protected void BlockZones(bool block)
    {
        ...
        {
            foreach (SpellZoneTag tag in this.m_ZonesToDisable)
            {
                List<Zone> list = SpellUtils.FindZonesFromTag(tag);
                if (list != null)
                {
    ...
    private void BuildFsmStateMap()
    {
        if (this.m_fsm != null)
        {
            ...
            foreach (KeyValuePair<SpellStateType, int> pair in map)
            {
                if (pair.Value > 1)
                {
            ...
            foreach (KeyValuePair<SpellStateType, int> pair2 in map2)
            {
                if (pair2.Value > 1)
                {
                ...
                if ((pair2.Value > 0) && (map[pair2.Key] == 0))
                {
    ...
    {
        return new <WaitThenChangeFsmState>c__Iterator28 { stateType = stateType, <$>stateType = stateType, <>f__this = this };
    }

    [CompilerGenerated]
    private sealed class <WaitThenChangeFsmState>c__Iterator28 : IDisposable, IEnumerator, IEnumerator<object>
    {
