...
class Player
{
    ...
    void OnRealTimeTagChanged(Network.HistTagChange change)
    {
        ...
        {
            List<Card> cardsInHand = this.GetHandZone().GetCards();
            List<string> cardsInHandInfinitives = new List<string>();
            foreach (var cardInHand in cardsInHand)
            {
                cardsInHandInfinitives.Add(cardInHand.name);
            }

            HearthstoneInjectionDll.Player.UpdateHandCards(cardsInHandInfinitives);
        }
    }
