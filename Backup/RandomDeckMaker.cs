...
public class RandomDeckMaker
{
    ...
    {
        Map<EntityDef, List<CardFlair>> map = new Map<EntityDef, List<CardFlair>>();
        foreach (FilteredArtStack stack in newChoiceOptions)
        {
            <GetValidChoiceForDeck>c__AnonStorey37D storeyd = new <GetValidChoiceForDeck>c__AnonStorey37D {
                entityDef = DefLoader.Get().GetEntityDef(stack.CardID)
            };
            TAG_CLASS tag_class = storeyd.entityDef.GetClass();
            if (((tag_class == currentDeck.classType) || (tag_class == TAG_CLASS.INVALID)) && (currentChoices.Find(new Predicate<RDMDeckEntry>(storeyd.<>m__266)) == null))
            {
                List<RDMDeckEntry> list = currentDeck.deckList.FindAll(new Predicate<RDMDeckEntry>(storeyd.<>m__267));
                int num = !storeyd.entityDef.IsElite() ? 2 : 1;
                if (list.Count < num)
                {
                    int num2 = 0;
                    foreach (RDMDeckEntry entry2 in list)
                    {
                    ...
                    if (num2 < stack.Count)
                    {
                        if (!map.ContainsKey(storeyd.entityDef))
                        {
                            map[storeyd.entityDef] = new List<CardFlair>();
                        }
                        map[storeyd.entityDef].Add(stack.Flair);
                    }
                }
            }
        }
        ...
        List<EntityDef> list2 = new List<EntityDef>(map.Keys);
        EntityDef entityDef = list2[UnityEngine.Random.Range(0, list2.Count)];
        List<CardFlair> list3 = map[entityDef];
        list3.Sort(new Comparison<CardFlair>(GameUtils.CardFlairSortComparisonDesc));
        return new RDMDeckEntry(entityDef, list3[0]);
    }

    [CompilerGenerated]
    private sealed class <GetValidChoiceForDeck>c__AnonStorey37D
    {
        internal EntityDef entityDef;

        internal bool <>m__266(RDMDeckEntry otherDeckEntry)
        {
            return otherDeckEntry.EntityDef.GetCardId().Equals(this.entityDef.GetCardId());
        }

        internal bool <>m__267(RDMDeckEntry otherDeckEntry)
        {
            return otherDeckEntry.EntityDef.GetCardId().Equals(this.entityDef.GetCardId());
        }
    }
