...
public class CollectionCoverDisplay : PegUIElement
{
    ...
    {
        return new <AnimateLatchCrackOpen>c__Iterator2A { <>f__this = this };
    }
    ...
    [CompilerGenerated]
    private sealed class <AnimateLatchCrackOpen>c__Iterator2A : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            {
                case 0:
                    this.<>f__this.m_bookCoverLatchJoint.GetComponent<Renderer>().material = this.<>f__this.m_latchOpaqueMaterial;
                    this.<>f__this.m_bookCoverLatch.GetComponent<Animation>()[this.<>f__this.LATCH_OPEN_ANIM_NAME].time = 0f;
                    this.<>f__this.m_bookCoverLatch.GetComponent<Animation>()[this.<>f__this.LATCH_OPEN_ANIM_NAME].speed = this.<>f__this.LATCH_OPEN_ANIM_SPEED;
                    SoundManager.Get().LoadAndPlay("collection_manager_book_latch_jiggle");
                    this.<>f__this.m_bookCoverLatch.GetComponent<Animation>().Play(this.<>f__this.LATCH_OPEN_ANIM_NAME);
                    break;
            ...
            if (this.<>f__this.m_bookCoverLatch.GetComponent<Animation>()[this.<>f__this.LATCH_OPEN_ANIM_NAME].time < 0.75f)
            {
                this.$current = null;
                this.$PC = 1;
                return true;
            }
            this.<>f__this.m_bookCoverLatch.GetComponent<Animation>()[this.<>f__this.LATCH_OPEN_ANIM_NAME].speed = 0f;
            this.$PC = -1;
        Label_0143:
            return false;
        }
