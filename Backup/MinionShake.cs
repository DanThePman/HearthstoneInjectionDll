...
public class MinionShake : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if (((((manager == null) || (manager.RenderQualityLevel != GraphicsQuality.Low)) && this.m_Animating) && (this.m_Animator != null)) && (this.m_MinionShakeInstance != null))
        {
            if ((this.m_Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == s_IdleState) && !this.m_Animator.GetBool("shake"))
            {
    ...
    {
        return new <ResetShakeAnimator>c__Iterator286 { <>f__this = this };
    }
    ...
    public static void ShakeAllMinions(GameObject shakeTrigger, ShakeMinionType shakeType, Vector3 impactPoint, ShakeMinionIntensity intensityType, float intensityValue, float radius, float startDelay)
    {
        foreach (MinionShake shake in FindAllMinionShakers(shakeTrigger))
        {
            ...
            if (events != null)
            {
                events.MinionShakeEvent(intensityType, intensityValue);
            }
        }
    }
    ...
    private void ShakeMinion()
    {
        if ((GraphicsManager.Get() != null) && (GraphicsManager.Get().RenderQualityLevel != GraphicsQuality.Low))
        {
            ...
            {
                UnityEngine.Debug.LogWarning("MinionShake: failed to locate MinionShake Animator");
            }
            else
            {
                ...
                if (((component == null) || !component.isPlaying) || this.m_IgnoreAnimationPlaying)
                {
                    ...
                    if (((zero.y - base.transform.position.y) <= 0.1f) || this.m_IgnoreHeight)
                    {
                        if (this.m_MinionShakeInstance == null)
                        {
    ...
    {
        ShakeObject(shakeObject, shakeType, impactPoint, intensityType, intensityValue, radius, startDelay, false);
    }
    ...
    {
        ShakeObject(shakeObject, shakeType, impactPoint, intensityType, intensityValue, radius, startDelay, false, ignoreAnimationPlaying);
    }
    ...
    public static void ShakeTargetMinion(GameObject shakeTarget, ShakeMinionType shakeType, Vector3 impactPoint, ShakeMinionIntensity intensityType, float intensityValue, float radius, float startDelay)
    {
        ...
        if (spell == null)
        {
            UnityEngine.Debug.LogWarning("MinionShake: failed to locate Spell component");
        }
        else
        {
            ...
            {
                UnityEngine.Debug.LogWarning("MinionShake: failed to Spell GetVisualTarget");
            }
            else
            {
                ...
                if (componentInChildren == null)
                {
                    UnityEngine.Debug.LogWarning("MinionShake: failed to locate MinionShake component");
                }
                else
                {
                    componentInChildren.m_StartDelay = startDelay;
    ...
    {
        return new <StartShakeAnimation>c__Iterator285 { <>f__this = this };
    }

    [CompilerGenerated]
    private sealed class <ResetShakeAnimator>c__Iterator286 : IDisposable, IEnumerator, IEnumerator<object>
    {
    ...
    [CompilerGenerated]
    private sealed class <StartShakeAnimation>c__Iterator285 : IDisposable, IEnumerator, IEnumerator<object>
    {
        internal object $current;
        internal int $PC;
        internal MinionShake <>f__this;
